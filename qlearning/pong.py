...
screen = pygame.display.set_mode((WINDOW_WITH, WINDOW_HEIGHT))
...
def drawPaddle(switch, paddleYpos):
    if switch ==  "left":
        ...
        paddle = pygame.Rect(WINDOW_WITH - PADDLE_BUFFER -PADDLE_WIDTH, paddleYpos, PADDLE_WIDTH, PADDLE_HEIGHT)
...
def updateBall(paddle1YPos, paddle2YPos, ballXPos, ballYPos, ballXDirection, ballYDirection, DeltaFrameTime):
    ...
    if ((ballXPos >= (WINDOW_WITH - PADDLE_WIDTH - PADDLE_BUFFER)) and ((ballYPos + BALL_HEIGHT)>= paddle2YPos) and (ballYPos <=(paddle2YPos+PADDLE_HEIGHT)) and (ballXDirection == 1)):
        ballXDirection = -1

    elif(ballXPos > WINDOW_WITH - BALL_WIDTH):
        ballXDirection = -1
...
class PongGame:
    ...
    def PlayNextMove(self, action):
        ...
        [score, self.ballXPos, self.ballYPos, self.ballXPos, self.ballYPos] = updateBall(self.paddle1YPos, self.paddle2YPos, self.ballXPos, self.ballYPos, self.ballXDirection,self.ballYDirection, DeltaFrameTime)
